...
# import sys
# sys.path.append('/root/autodl-tmp/timesfm/src')
# print(sys.path)
from adapter_new.utils import get_adapter_params , load_adapter_checkpoint,load_adapter_layer
from adapter_new.remove_file import clean_folders
# from adapter.utils import get_adapter_params, load_adapter_layer
# from adapter import utils
from timesfm import TimesFm, data_loader, patched_decoder,TimesFmHparams,TimesFmCheckpoint
import os
...
def finetune(
    *,
    ...
    datetime_col: Annotated[str, typer.Option(help="Column having datetime.")] = "data_time",
    ts_cols: Annotated[
    ...
    context_len: Annotated[int, typer.Option(help="Length of the context window")] = 512,
    horizon_len: Annotated[int, typer.Option(help="Prediction length.")] = 127,
    freq: Annotated[
        str,
        typer.Option(
            ...,
    ...
    data_path: Annotated[str, typer.Option(help="Path to dataset csv")] = '/root/autodl-tmp/timesfm/datasets/gnrx/weather_long_power_dropnull.csv',
    ...
    backend: Annotated[str, typer.Option(help="Backend device: cpu, gpu, tpu")] = 'gpu',
    ...
    learning_rate: Annotated[float, typer.Option(help="adam optimizer learning rate")] = 0.001,
    adam_epsilon: Annotated[float, typer.Option(help="adam optimizer epsilon")] = 1e-7,
    ...
    cos_decay_steps: Annotated[int, typer.Option(help="Number of cosine decay steps")] = 1,
    ema_decay: Annotated[float, typer.Option(help="Exponential moving average decay")] = 0.9999,
    ...
    ] = "/root/autodl-tmp/timesfm/ckpt/16",
    ...
    if boundaries == (0, 0, 0):
        ...
        boundaries = [
            int(len(data_df) * 0.88),
            int(len(data_df) * 0.966),
            len(data_df) - 1,
        ]
        print(boundaries)
    # ts_cols = [col for col in data_df.columns if col != datetime_col ]
    
    # ts_cols = [col for col in data_df.columns if col != datetime_col and col.startswith('predict') and col != 'predicted_power' or col == 'real_power']
    # num_cov_cols = [col for col in ts_cols if not(col.endswith('power')) ]
    
    num_cov_cols = [col for col in data_df.columns if col != datetime_col and col.startswith('predict') and col != 'predicted_power']
    ts_cols = ['real_power']
    ...
    #     tfm.load_from_checkpoint(
    #         checkpoint=TimesFmCheckpoint(
    #                      path = '/root/autodl-tmp/timesfm-1.0-200m/checkpoints/'            
    #                     # huggingface_repo_id="google/timesfm-1.0-200m"
    #                     )
    #     )
